...
device_num = 0
model_num = 2
nnet = ARGUS_ptx_ar_train("../ARGUS/ARGUS_ptx_ar.cfg", "vfold", device_num=device_num)
nnet.setup_vfold_files()
for vfn in range(nnet.num_folds):
    nnet.setup_training_vfold(vfn, run_num)
    pretrained_model_name = "./Pretrained_Models/pretrain_half_vfold_run"+str(run_num)+"/best_model_"+str(model_num)+".pth"
