...
class ARGUS_preprocess_lumify():
    ...
    def get_depth_and_zoom(self, im):
        ...
        assert len(y) > 5, "Could not find ruler in Lumify format."
        assert len(y) < 75, "Could not find ruler in Lumify format."
        ...
        assert len(yCenters) > 5, "Could not find ruler in Lumify format."
        assert len(yCenters) < 75, "Could not find ruler in Lumify format."
    ...
    def process(self, vid_img):
        ...
        filename = path.join(path.dirname(__file__), 'linearization_maps_lumify', f'linear_map_depth{str(depth)}.npy')
